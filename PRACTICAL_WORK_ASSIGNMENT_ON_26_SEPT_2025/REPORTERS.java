...
public class REPORTERS extends JPanel implements ActionListener {
    ...
    public REPORTERS(String role, int userId) {
        this.currentRole = role;
        this.currentUserId = userId;
        setLayout(null);

        // Table
        String[] columns = {"ReporterID", "Name", "Email", "Organization"};
        ...
        try (Connection con = DB.getConnection()) {
            if (e.getSource() == addBtn) {
                PreparedStatement ps = con.prepareStatement(
                        "INSERT INTO Reporters(Name, Email, Organization) VALUES(?,?,?)");
                ps.setString(1, nameTxt.getText());
                ps.setString(2, emailTxt.getText());
                ps.setString(3, organizationTxt.getText());
                ps.executeUpdate();
                JOptionPane.showMessageDialog(this, "Reporter Added!");
                loadReporters();
            } else if (e.getSource() == updateBtn) {
                if (idtxt.getText().isEmpty()) return;
                PreparedStatement ps = con.prepareStatement(
                        "UPDATE Reporters SET Name=?, Email=?, Organization=? WHERE ReporterID=?");
                ps.setString(1, nameTxt.getText());
                ps.setString(2, emailTxt.getText());
                ps.setString(3, organizationTxt.getText());
                ps.setInt(4, Integer.parseInt(idtxt.getText()));
                ps.executeUpdate();
                JOptionPane.showMessageDialog(this, "Reporter Updated!");
                loadReporters();
            } else if (e.getSource() == deleteBtn) {
                if (idtxt.getText().isEmpty()) return;
                PreparedStatement ps = con.prepareStatement("DELETE FROM Reporters WHERE ReporterID=?");
    ...
    private void loadReporters() {
        try (Connection con = DB.getConnection()) {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM Reporters");
            while (rs.next()) {
